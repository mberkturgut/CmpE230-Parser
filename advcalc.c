   ...
   //read the line & tokenize
    while (fgets(input_line, sizeof(input_line), stdin) != NULL){
        ...
        while(*q != '\0'){
            ...
            else if (*q == '%'){ //rest is comments, don't read
                break;
            }
            else { //other things like '+' or '('
                strncpy(tokens[numtokens],q,1);
        ...
        if(numtokens > 1) { //only parse if the tokens array is not empty. input_line " %... ", "", "\t" -> tokens empty
...
void evaluator (char *str) //calculates the parsed input stored in str
{
   ...
   while (true) {
      ...
      if (strcmp(elm, "=") == 0) { //if the operation is variable assignment (stored in this postfix format: 2 x =)
            ...
            break; //assignment is the last operation
         }
...
int parse_integer(char *token) {
    int isnumber = 1 ;
    char int_str[TOKENLENGTH] = "";
    for(q = token ; *q != '\0' ; q++) {
        isnumber = isnumber && isdigit(*q) ;
        if (isnumber)
            strncat(int_str,q,1);
    ...
    strcpy(tokens[numtokens],int_str);
    numtokens++;
    return(isnumber) ; 
}    
...
int parse_variable(char *token) {
    ...
    strcpy(tokens[numtokens],var_str);
    numtokens++;
    return(isvariable) ; 
}    
